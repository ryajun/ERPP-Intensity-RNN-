...
def make_model(d_model=128, dropout=0.1, process_dim=10,
               device = 'cpu', pe='concat', max_sequence_length=4096):
    "helper: construct a models form hyper parameters"

    model = SAHP( d_model, dropout=dropout, process_dim=process_dim, device = device,
...
def prediction_evaluation(device, model, test_seq_lengths, test_seq_times, test_seq_types, test_size, tmax):
    ...
    from sklearn.metrics import confusion_matrix, accuracy_score, f1_score, recall_score, \
        roc_auc_score, precision_score  # 混淆矩阵主要用于比较分类结果和实例的真实信息。矩阵中的每一行代表实例的预测类别，每一列代表实例的真实类别
    types_predict_score = f1_score(types_real, types_estimates, average='micro')  # preferable in class imbalance
    types_predict_precision = precision_score(types_real, types_estimates, average='macro')
    types_predict_accuracy = accuracy_score(types_real, types_estimates)
    types_predict_recall = recall_score(types_real, types_estimates, average='macro')
    # types_predict_auc = roc_auc_score(types_real, types_estimates,average='micao')
    print("Type prediction score:", types_predict_score)
    print("Type prediction accuracy:", types_predict_accuracy)
    print("Type prediction recall:", types_predict_recall)
    print("Type prediction precision:", types_predict_precision)
    # print("Type prediction auc:", types_predict_auc)

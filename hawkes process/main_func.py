...
DEFAULT_BATCH_SIZE =2#原本不是10 忘了是多少了
DEFAULT_HIDDEN_SIZE = 16
DEFAULT_LEARN_RATE = 1e-3#学习率
...
parser.add_argument('-t', '--task', type=str, default='mimic',
                    help = 'task type')
...
SYNTH_DATA_FILES = glob.glob("D:/data/simulated/*.pkl")
TYPE_SIZE_DICT = {'cov': 56, 'bookorder':8, 'meme':5000, 'mimic':75, 'stackOverflow':22,
                  'synthetic':3}
...
if __name__ == '__main__':
    ...
    if args.task in SYNTHETIC_TASKS:
        ...
        test_seq_times, test_seq_types, test_seq_lengths, test_tmax = \
            process_loaded_sequences(test_hawkes_data, process_dim)#看下process_loaded_sequences模块

...
class SAHP(nn.Module):
    ...
    def __init__(self,  d_model, dropout, process_dim, device, max_sequence_length):
        ...
        # self.event_emb = EventEmbedding(self.input_size, d_model)
        ...
        self.intensity_w = torch.nn.Parameter(torch.FloatTensor(2,d_model))
        self.intensity_b = torch.nn.Parameter(torch.FloatTensor(2,d_model))
        ...
        self.event_linear = nn.Linear(in_features=16, out_features=75)

        self.rnn = nn.RNN(input_size=16, hidden_size=16,
                      batch_first=True, num_layers=3, bidirectional=False, nonlinearity='relu')

        # self.event_linear = nn.Linear(in_features=16, out_features=16, bias=True)
        self.event_linear = nn.Linear(in_features=16, out_features=16, bias=True)

    def state_decay(self,  start_point, omega, duration_t):
        # * element-wise product
        cell_t = torch.tanh(start_point + torch.exp(- omega * duration_t))
        return cell_t

    def intensities(self,time_var, time_cif, w, b):
        ints = torch.tanh(time_cif + torch.exp(w * time_var) + b)
        return ints
    ...
    def compute_loss(self, seq_times, seq_onehot_types,n_mc_samples = 20):
        ...
        Shape:
            one-element tensor
        """
        time_cif = self.embed_info
        target_size_x, target_size_y, _ = time_cif.size()
        intensity_w = self.intensity_w
        # intensity_w = intensity_w.unsqueeze(0)
        # intensity_w = intensity_w.unsqueeze(1)
        intensity_w = intensity_w.unsqueeze(1).repeat(1, target_size_y, 1)
        intensity_b = self.intensity_b

        intensity_b = intensity_b.unsqueeze(1).repeat(1, target_size_y, 1)

        dt_seq = seq_times[:, 1:] - seq_times[:, :-1]
        # cell_t = self.state_decay(self.start_point, self.omega, dt_seq[:, :, None])
        # cell_t = self.embed_info
        ...
        intens = self.intensities(dt_seq[:, :, None], time_cif, intensity_w, intensity_b)
        intens_at_evs = self.intensity_layer(intens)
        ...
        cell_tau = intens.unsqueeze(3)
        cell_t = cell_tau * taus
        cell_t = cell_t.transpose(2, 3)
        intens_at_samples = self.intensity_layer(cell_t).transpose(2,3)
    ...
    def read_predict(self, seq_times, seq_types, seq_lengths, pad, device,
        ...
        with torch.no_grad(): #前向传播后不会进行求导和进行反向传播
            ...
            if self.embed_info.shape[1] == 0:  # only one element shape[1]表示矩阵的列数[0]表示行数
            ...
            #     omega = torch.squeeze(self.omega)[None, :]
                ...
                # time_cif = torch.squeeze(self.embed_info)[-1, :]
                time_cif = self.embed_info
                time_cif = time_cif.squeeze(0)
                time_cif = time_cif[-1,:]
                intensity_w = torch.squeeze(self.intensity_w)[-1, :]
                intensity_b = torch.squeeze(self.intensity_b)[-1, :]

            dt_vals = torch.linspace(0, hmax, n_samples + 1).to(device) #linspace返回一个1维张量，包含在区间start和end上均匀间隔的step个点

            h_t_vals = self.intensities(dt_vals[:, None], time_cif, intensity_w, intensity_b)
            # h_t_vals = self.state_decay(
            #                             start_point,
            #                             omega,
            #                             dt_vals[:, None])
            if print_info:
